...
public class Bound implements ConfigurationSerializable {
	...
	public Bound( int x, int y, int z, int x2, int y2, int z2, String world, List<Entity> entities, String boundName) {
		this.world = world;
		this.x = Math.min(x,x2);
		this.y = Math.min(y, y2);
		this.z = Math.min(z, z2);
	...
	public Map<String, Object> serialize() {
		...
		serialized.put("y", y);
		serialized.put("z", z);
		serialized.put("x2", x2);
		serialized.put("y2", y2);
		serialized.put("z2", z2);
		serialized.put("world", world);
		serialized.put("entities", entities);
		serialized.put("boundName", boundName);
		return serialized;
	}

	public static Bound deserialize(Map<String, Object> deserialize) {
		return new Bound(
				(int) deserialize.get("x"),
				(int) deserialize.get("y"),
				(int) deserialize.get("z"),
				(int) deserialize.get("x2"),
				(int) deserialize.get("y2"),
				(int) deserialize.get("z2"),
				(String) deserialize.get("world"),
				(List<Entity>) deserialize.get("entities"),
				(String) deserialize.get("boundName")
		);
	}
//
//	private int x;
