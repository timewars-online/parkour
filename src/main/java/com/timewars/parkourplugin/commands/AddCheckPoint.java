...
public class AddCheckPoint implements TabExecutor {
    ...
    public AddCheckPoint(ParkourPlugin parkourPlugin) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if ( sender instanceof Player)
        {
            ...
            // which blocks are checkpoints
            if (!parkourPlugin.getCheckPointMaterials().contains(targetBlock.getType()) )
            {
                p.sendMessage("Please select only" + ChatColor.BOLD + "plates" + ChatColor.AQUA + " as checkpoints!");
                return true;
            }

            if (parkourPlugin.getParkours().get(parkourName).getCheckpoints().contains(targetBlock.getLocation()) )
            {
                p.sendMessage("This is already a checkpoint! Command to" + ChatColor.BOLD + "remove" + " a checkpoint is " + ChatColor.AQUA + "removecheckpoint");
                return true;
            }
            parkourPlugin.getParkours().get(parkourName).getCheckpoints().add(targetBlock.getLocation());
            p.sendMessage("Successfully added checkpoint to " + ChatColor.DARK_RED + parkourName +
