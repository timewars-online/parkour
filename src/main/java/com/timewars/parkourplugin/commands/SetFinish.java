...
public class SetFinish implements TabExecutor {

    ParkourPlugin parkourPlugin;

    public SetFinish(ParkourPlugin parkourPlugin) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if( sender instanceof Player)
        {
            ...
            try {
                ...
                bound.setBoundName("Finish");
                parkourPlugin.getParkours().get(parkourPlugin.getParkoursInEditing().get(p)).setFinish(bound);
            } catch (Exception e) {
                p.sendMessage("You have to select a region to add it!");
            }
            p.sendMessage("Successfuly added a region called " + ChatColor.AQUA + "Finish" + "!");
            return true;
        }
        return false;
    }
