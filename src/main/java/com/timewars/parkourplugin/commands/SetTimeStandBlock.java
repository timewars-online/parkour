...
public class SetTimeStandBlock implements TabExecutor {

    ParkourPlugin parkourPlugin;

    public SetTimeStandBlock(ParkourPlugin parkourPlugin) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if( sender instanceof Player)
        {
            ...
            parkour.setTimeStandBlock(Float.valueOf(args[0]));
            p.sendMessage("The stand block time of " +  ChatColor.AQUA + parkour.getName() + " now is " + ChatColor.BOLD + parkour.getTimeStandBlock() );
            p.sendMessage("To enable deathrun use " + ChatColor.AQUA + "settype deathrun");
